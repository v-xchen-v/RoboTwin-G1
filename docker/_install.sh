...
SAPIEN_LOCATION=$(/opt/conda/bin/pip show sapien | grep 'Location' | awk '{print $2}')/sapien
...
MPLIB_LOCATION=$(/opt/conda/bin/pip show mplib | grep 'Location' | awk '{print $2}')/mplib
